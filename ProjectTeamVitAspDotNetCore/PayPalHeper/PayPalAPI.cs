...
namespace ProjectTeamVitAspDotNetCore.PayPalHeper
{
    public class PayPalAPI
    {
        ...
        public async Task<string> getRedirectURLToPayPal(double total, string currency)
        {
            try
            {
                return Task.Run(async () =>
                {
                    ...
                    PayPalPaymentCreateResponse createdPayment = await CreatePaypalPaymentAsync(http, accessToken,total,currency); 
                    return createdPayment.links.First(x => x.rel == "approval_url").href;
                }).Result;
            }
            catch(Exception ex)
            {
                Debug.WriteLine(ex, "Faild to login to PayPal");
                return null;
            }
        }
        ...
        private async Task<PayPalPaymentCreateResponse>CreatePaypalPaymentAsync(HttpClient http, PayPalAccessToken accessToken , double total,string currency)
        {
            ...
            string content = await response.Content.ReadAsStringAsync();
            PayPalPaymentCreateResponse paypalPaymentCreated = JsonConvert.DeserializeObject<PayPalPaymentCreateResponse>(content);
